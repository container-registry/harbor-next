...
// TestPingRegistryByIDIgnoresOverrides guards against credential exfiltration:
// a caller referencing an existing registry by id must not be able to override
// its saved connection settings and redirect saved credentials to an untrusted endpoint.
func (suite *RegistryTestSuite) TestPingRegistryByIDIgnoresOverrides() {
	...
	suite.Equal("saved-ca", pinged.CACertificate)
}
