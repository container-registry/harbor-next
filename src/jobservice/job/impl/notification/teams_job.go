...
func (tj *TeamsJob) Validate(params job.Parameters) error {
	...
	if address == nil {
		return errors.Errorf("malformed job parameter 'address', got nil")
	}
	str, ok = address.(string)
	if !ok || str == "" {
		return errors.Errorf("malformed job parameter 'address', expecting string but got %s", fmt.Sprintf("%T", address))
	}
	return nil
}
...
func (tj *TeamsJob) Run(ctx job.Context, params job.Parameters) error {
	...
	if err != nil {
		tj.logger.Errorf("exit teams job, error: %s", err)
	} else {
		tj.logger.Info("success to run teams job")
	}
	// Wait a second for teams rate limit
	time.Sleep(time.Second)
	return err
}

// init teams job
func (tj *TeamsJob) init(ctx job.Context, params map[string]any) error {
...
// execute teams job
func (tj *TeamsJob) execute(params map[string]any) error {
