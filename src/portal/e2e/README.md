...
```bash
bun x playwright test trivy.spec.ts
bun x playwright test webhook.spec.ts
```
...
# Then run tests with
WEBHOOK_ENDPOINT_UI=localhost:8084 bun x playwright test webhook.spec.ts
```
...
- Use Playwright Inspector: `bun x playwright test --debug`
