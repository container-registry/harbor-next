...
// resource types are derived from the "<action>_<resourceType>" shape of
// pkg/auditext/model.EventTypes (e.g. "create_artifact" -> "artifact").
// Keep this list in sync with that Go source when a new event type ships.
const AUDIT_EVENT_CATEGORIES: Omit<AuditEventCategory, 'events'>[] = [
    ...
    {
        id: 'members',
        labelKey: 'AUDIT_LOG_CONFIG.MEMBERS',
        icon: 'group',
...
export class AuditLogConfigurationComponent implements OnInit {
    ...
    isValid(): boolean {
        return !!this.auditConfigForm?.valid;
    }
    ...
    onForwardEndpointInput(): void {
        if (!this.currentConfig.audit_log_forward_endpoint.value) {
    ...
    private getActionOrder(action: string): number {
        const actionOrder = ['create', 'update', 'delete', 'pull', 'login'];
