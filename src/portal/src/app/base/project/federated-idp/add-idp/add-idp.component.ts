...
export class AddIdpComponent implements AfterViewChecked, OnDestroy, OnInit {
    ...
    // Project context - set by parent component
    projectId: number;
    ...
    areClaimsValid(): boolean {
        ...
        // Check required claims exist (aud, iss)
        ...
        for (const claim of this.claims) {
            ...
            // Validate path format (skip for mandatory claims like aud, iss)
            if (path && !this.isValidClaimPath(path)) {
                // Check if it's a mandatory claim (aud, iss are always valid)
                if (
                    path.toLowerCase() !== 'aud' &&
                    path.toLowerCase() !== 'iss'
                ) {
                    return false;
                }
            }
    ...
    reset(): void {
        ...
        // Set project_id after init if available
        if (this.projectId) {
            this.target.project_id = this.projectId;
        }
    ...
    addIdp() {
        ...
        // Ensure project_id is set for project-level IdPs
        if (this.projectId) {
            this.target.project_id = this.projectId;
        }
