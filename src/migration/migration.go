...
func Migrate(database *models.Database) error {
	...
	pool := dao.GetPool()
	if pool == nil {
		return fmt.Errorf("apply versioned Harbor Next schema: database pool is not initialized")
	}
	start = time.Now()
	if err := applyVersionedSchema(context.Background(), sqlVersionedSchemaDB{db: pool.DB()}, versionedSchemaDir()); err != nil {
		return err
	}
	observeMigrationPhase("versioned", start)
	return nil
}
