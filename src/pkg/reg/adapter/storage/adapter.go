...
func (a *adapter) FetchArtifacts(filters []*model.Filter) ([]*model.Resource, error) {
	...
	for i, r := range repositories {
		...
		runner.AddTask(func() error {
			...
			if err != nil {
				return nil
				// return fmt.Errorf("unable to get all tags for repo %s: %v: %v", r, tags, err)
			}
...
func (a *adapter) DeleteManifest(repository, tag string) error {
	ctx := context.Background()

	named, err := reference.WithName(repository)
	if err != nil {
		return fmt.Errorf("ref %s error: %v", repository, err)
	}
	repo, err := a.registry.Repository(ctx, named)
	if err != nil {
		return fmt.Errorf("unable to get repo %s: %v", repository, err)
	}
	tags := repo.Tags(ctx)
	return tags.Untag(ctx, tag)

	// return repo.Tags(ctx).Untag(ctx, tag)
	//
	// repo, err := a.getRepo(ctx, repository, tag)
	// if err != nil {
	// 	return err
	// }
	//
...
func (a *adapter) PullBlobChunk(repository, d string, _ int64, start, end int64) (size int64, blob io.ReadCloser, err error) {
	ctx := context.Background()
	...
	_, err = readSeeker.Seek(start, io.SeekStart)
	if err != nil {
		_ = readSeeker.Close()
	...
	if end < start {
		_ = readSeeker.Close()
		return 0, nil, fmt.Errorf("invalid blob chunk range %d-%d", start, end)
	}
	chunkSize := end - start + 1
	data, err := io.ReadAll(io.LimitReader(readSeeker, chunkSize))
	_ = readSeeker.Close()
	if err != nil {
		return 0, nil, fmt.Errorf("unable to read blob chunk: %v", err)
	}
	if int64(len(data)) != chunkSize {
		return 0, nil, fmt.Errorf("blob chunk size is %d, expected %d", len(data), chunkSize)
	}
...
func (a *adapter) PushBlobChunk(repository, d string, size int64, chunk io.Reader, start, _ int64, location string) (nextUploadLocation string, endRange int64, err error) {
	...
	if start == 0 {
		...
		a.uploads.Store(writer.ID(), writer)
	} else if cached, ok := a.uploads.Load(location); ok {
		writer = cached.(distribution.BlobWriter)
	} else {
		writer, err = repo.Blobs(ctx).Resume(ctx, location)
		if err != nil {
			return location, start - 1, fmt.Errorf("unable to resume blob: %v", err)
		}
		a.uploads.Store(writer.ID(), writer)
	}

	if writer.Size() != start {
		return writer.ID(), writer.Size() - 1, fmt.Errorf("unable to resume blob: upload offset is %d, expected %d", writer.Size(), start)
	}

	_, err = writer.ReadFrom(chunk)
	if err != nil {
		return writer.ID(), writer.Size() - 1, fmt.Errorf("unable to read from chunk: %v", err)
	}

	if writer.Size() < size {
		// another chunk needed
		return writer.ID(), writer.Size() - 1, nil
	}

	// done
	a.uploads.Delete(writer.ID())
...
func (a *adapter) ListReferrers(_, _, _ string) (*ocispec.Index, map[string][]string, error) {
	return nil, nil, fmt.Errorf("ListReferrers is not implemented")
}
