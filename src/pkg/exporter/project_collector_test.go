...
func (c *ProjectCollectorTestSuite) TestProjectCollector() {
	...
	// testPro3 has no quota records (simulates quota_per_project_enable=false).
	// Verify it is still included in the map rather than being silently dropped
	// and logged as "project not found". Quota defaults to the unlimited
	// convention (-1) so dashboards do not divide by zero or fire false
	// "quota exhausted" alerts; usage defaults to empty (0).
	...
	c.Equalf(pMap[testPro3.ProjectID].Quota, "{\"storage\": -1}", "project without quota record should default to unlimited (-1)")
