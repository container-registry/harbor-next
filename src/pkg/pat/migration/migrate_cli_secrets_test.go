...
func (suite *MigrateCliSecretsTestSuite) TestMigrateCliSecretsMultipleUsers() {
	...
	// Verify each of this test's users got a legacy PAT. ClearTables only
	// runs at TearDownSuite, so other tests in this suite may have their
	// own "cli-secret" PATs still present — scope the check to these users
	// rather than counting every "cli-secret" PAT in the table.
	for _, uid := range userIDs {
		pats, err := suite.patMgr.List(ctx, q.New(q.KeyWords{"user_id": uid, "name": "cli-secret"}))
		suite.NoError(err)
		suite.Require().Equal(1, len(pats))
		suite.Equal("cli-secret", pats[0].Name)
		suite.True(pats[0].IsLegacy)
	}
}
...
func (suite *MigrateCliSecretsTestSuite) TestMigrateCliSecretsNoExistingSecrets() {
	...
	// Verify no PAT was created for this user (other tests in this suite
	// may have their own "cli-secret" PATs still present, since ClearTables
	// only runs at TearDownSuite).
	pats, err := suite.patMgr.List(ctx, q.New(q.KeyWords{"user_id": uid}))
	suite.NoError(err)
	suite.Equal(0, len(pats))
}
