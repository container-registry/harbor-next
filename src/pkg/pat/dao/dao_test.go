...
func (suite *DAOTestSuite) SetupSuite() {
	...
	// personal_access_token.user_id has a foreign key to harbor_user.
	// user_id 1 and 2 are pre-seeded by the base schema (admin, anonymous).
	for userID := 1; userID <= 7; userID++ {
		suite.ExecSQL(
			"INSERT INTO harbor_user (user_id, username, email, password, realname) VALUES (?, ?, ?, ?, ?) ON CONFLICT (user_id) DO NOTHING",
			userID, fmt.Sprintf("daotestuser%d", userID), fmt.Sprintf("daotestuser%d@example.com", userID), "Harbor12345", fmt.Sprintf("DAO Test User %d", userID),
		)
	}
}
...
func (suite *DAOTestSuite) TestGet() {
	...
	// Create first; id is auto-generated (BIGSERIAL) and not written back
	// onto pat, so use the id Create returns rather than assuming any
	// particular value.
	id, err := suite.dao.Create(suite.Context(), pat)
	suite.NoError(err)
