...
type DAO interface {
	...
	ListBySecretPrefix(ctx context.Context, userID int, secretPrefix string, disabled bool) ([]*model.PersonalAccessToken, error)
}
...
func (d *dao) ListBySecretPrefix(ctx context.Context, userID int, secretPrefix string, disabled bool) ([]*model.PersonalAccessToken, error) {
	...
	_, err = ormer.Raw(
		"SELECT * FROM personal_access_token WHERE user_id = ? AND secret_prefix = ? AND disabled = ?",
		userID, secretPrefix, disabled,
	).QueryRows(&pats)
	if err != nil {
		return nil, err
	}
