...
func (a *abstractor) AbstractMetadata(ctx context.Context, artifact *artifact.Artifact) error {
	...
	ma := GetManifestAbstractor(manifestMediaType)
	if ma == nil {
		return fmt.Errorf("unsupported manifest media type: %s", manifestMediaType)
	}
	if err := ma.AbstractManifest(ctx, artifact, content); err != nil {
		return err
	}
