...
func (a *abstractor) AbstractMetadata(ctx context.Context, artifact *artifact.Artifact) error {
	...
	manifestAbstractor := getManifestAbstractor(a, artifact.ManifestMediaType)
	if manifestAbstractor == nil {
		return fmt.Errorf("unsupported manifest media type: %s", artifact.ManifestMediaType)
	}
	if err := manifestAbstractor.AbstractMetadata(ctx, artifact, content); err != nil {
		return err
	}
