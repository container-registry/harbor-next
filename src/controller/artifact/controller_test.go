...
func (c *controllerTestSuite) TestAssembleArtifactFiltersLegacyBuildKitReferences() {
	legacyRef := &artifact.Reference{
		ID:          2,
		ChildID:     3,
		ChildDigest: "sha256:legacy-attestation",
		Platform:    &v1.Platform{Architecture: "unknown", OS: "unknown"},
		Annotations: map[string]string{
			buildKitReferenceTypeAnnotation:   buildKitAttestationManifestType,
			buildKitReferenceDigestAnnotation: "sha256:linux-amd64",
		},
	}
	normalRef := &artifact.Reference{
		ID:          1,
		ChildID:     2,
	...
	art := &artifact.Artifact{
		...
		References:        []*artifact.Reference{normalRef, legacyRef},
	}
	c.blobMgr.On("GetByArt", mock.Anything, legacyRef.ChildDigest).Return([]*models.Blob{{ContentType: inTotoLayerMediaType}}, nil)

	assembled := c.ctl.assembleArtifact(context.Background(), art, nil)
	c.Require().Len(assembled.References, 1)
	c.Equal(normalRef.ChildDigest, assembled.References[0].ChildDigest)
}

func (c *controllerTestSuite) TestPopulateAccessoriesSynthesizesLegacyBuildKitAttestations() {
	childArt := &Artifact{Artifact: artifact.Artifact{
		ID:                2,
		ManifestMediaType: v1.MediaTypeImageManifest,
		RepositoryName:    "library/hello-world",
		Digest:            "sha256:linux-amd64",
	}}
	parentRef := &artifact.Reference{ParentID: 1, ChildID: 2, ChildDigest: childArt.Digest}
	legacyRef := &artifact.Reference{
		ID:          3,
		ChildID:     4,
		ChildDigest: "sha256:legacy-attestation",
		...
		Annotations: map[string]string{
			buildKitReferenceTypeAnnotation:   buildKitAttestationManifestType,
			buildKitReferenceDigestAnnotation: childArt.Digest,
		},
	}
	parentArt := &artifact.Artifact{
		ID:                1,
		Type:              image.ArtifactTypeImage,
		ManifestMediaType: v1.MediaTypeImageIndex,
		References:        []*artifact.Reference{legacyRef},
	}
	attestationArt := &artifact.Artifact{
		ID:             4,
		RepositoryName: childArt.RepositoryName,
		Digest:         legacyRef.ChildDigest,
		Size:           321,
	}

	c.accMgr.On("List", mock.Anything, mock.Anything).Return([]accessorymodel.Accessory{}, nil)
	c.artMgr.On("ListReferences", mock.Anything, mock.Anything).Return([]*artifact.Reference{parentRef}, nil)
	c.artMgr.On("Get", mock.Anything, parentArt.ID).Return(parentArt, nil)
	c.artMgr.On("Get", mock.Anything, attestationArt.ID).Return(attestationArt, nil)
	c.blobMgr.On("GetByArt", mock.Anything, legacyRef.ChildDigest).Return([]*models.Blob{{ContentType: inTotoLayerMediaType}}, nil)

	c.ctl.populateAccessories(context.Background(), childArt)
	c.Require().Len(childArt.Accessories, 1)
	c.Equal(accessorymodel.TypeBuildKitAttestation, childArt.Accessories[0].GetData().Type)
	c.Equal(attestationArt.ID, childArt.Accessories[0].GetData().ArtifactID)
	c.Equal(childArt.Digest, childArt.Accessories[0].GetData().SubArtifactDigest)
}
