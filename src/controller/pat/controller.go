...
type Controller interface {
	...
	// ListBySecretPrefix returns non-disabled PATs matching user and secret prefix
	ListBySecretPrefix(ctx context.Context, userID int, secretPrefix string) ([]*model.PersonalAccessToken, error)
...
func (c *controller) Create(ctx context.Context, pat *model.PersonalAccessToken) (int64, string, error) {
	...
	// Generate a random plaintext secret using robot account's CreateSec
	// utility, but hash it with the stronger, PAT-specific scheme rather
	// than the hash CreateSec returns (which uses the legacy iteration
	// count shared with robot secrets).
	_, plaintextSecret, salt, err := robot.CreateSec()
	if err != nil {
		return 0, "", err
	}
	secret := utils.HashPATSecret(plaintextSecret, salt)
	...
	patToCreate := &model.PersonalAccessToken{
		...
		Secret:       secret,
		Salt:         salt,
		SecretPrefix: fmt.Sprintf("%x", sha256.Sum256([]byte(plaintextSecret)))[:8],
		ExpiresAt:    expiresAt,
		Disabled:     false,
		IsLegacy:     false,
		Scope:        scope,
...
func (c *controller) intersectScope(ctx context.Context, userID int, userScope string) (string, error) {
	...
	// computeScope can emit multiple AccessLevel entries for the same
	// resource (e.g. a separate entry for "pull" and one for "push"), so
	// merge actions per resource instead of letting a later entry
	// overwrite an earlier one for the same resource.
	...
	for _, ps := range fullScopes {
		...
		for _, al := range ps.Access {
			actions, ok := actionsByResource[al.Resource]
			if !ok {
...
// ListBySecretPrefix returns non-disabled PATs matching user and secret prefix
func (c *controller) ListBySecretPrefix(ctx context.Context, userID int, secretPrefix string) ([]*model.PersonalAccessToken, error) {
	return c.patMgr.ListBySecretPrefix(ctx, userID, secretPrefix, false)
}
...
func (c *controller) RefreshSecret(ctx context.Context, id int64, newSecret string) (string, error) {
	...
	if newSecret != "" {
		...
		secret = utils.HashPATSecret(newSecret, pat.Salt)
	} else {
		// Generate a new random plaintext secret; hash it with the
		// PAT-specific scheme rather than the hash CreateSec returns.
		_, genSecret, _, genErr := robot.CreateSec(pat.Salt)
		if genErr != nil {
			return "", genErr
		}
		plaintextSecret = genSecret
		secret = utils.HashPATSecret(genSecret, pat.Salt)
	}

	// Update the PAT with the new secret
	pat.Secret = secret
	pat.SecretPrefix = fmt.Sprintf("%x", sha256.Sum256([]byte(plaintextSecret)))[:8]
	if err := c.patMgr.Update(ctx, pat, "secret", "secret_prefix"); err != nil {
		return "", err
	}
