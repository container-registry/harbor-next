...
func (suite *ControllerTestSuite) SetupSuite() {
	...
	// Create test users for PAT tests. user_id 1 and 2 are pre-seeded by
	// the base schema (admin, anonymous), so skip inserting over them.
	for userID := 1; userID <= 12; userID++ {
		username := fmt.Sprintf("testuser%d", userID)
		email := fmt.Sprintf("user%d@example.com", userID)
		realname := fmt.Sprintf("Test User %d", userID)
		suite.ExecSQL("INSERT INTO harbor_user (user_id, username, email, password, realname) VALUES (?, ?, ?, ?, ?) ON CONFLICT (user_id) DO NOTHING", userID, username, email, "Harbor12345", realname)
	}

	// computeScope/intersectScope derive a PAT's scope from actual project
	// membership, so scope-related tests need a real project + membership
	// to compute against. A running Harbor bootstraps a "library" project
	// at startup; this bare migrated-schema test DB doesn't have one.
	suite.ExecSQL("INSERT INTO project (project_id, owner_id, name) VALUES (1, 1, 'library') ON CONFLICT (project_id) DO NOTHING")
	suite.ExecSQL("INSERT INTO project_member (project_id, entity_id, entity_type, role) VALUES (1, 1, 'u', ?) ON CONFLICT (project_id, entity_id, entity_type) DO NOTHING", common.RoleProjectAdmin)
}
