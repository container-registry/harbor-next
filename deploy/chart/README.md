...
- Typo detection: unknown **top-level** keys are rejected
  (`additionalProperties: false` at the root). Component config blocks
  stay open by design — the generic passthrough means new application
  config keys never require a chart change.
...
| database.connMaxIdleTime | string | `"300s"` | Max time a connection may sit idle before the pool closes it (empty/0 keeps the pool's built-in default). |
| database.connMaxLifetime | string | `"30m"` | Max total lifetime before a connection is recycled, useful for failover/LB hygiene (empty/0 keeps the pool's built-in default). |
...
| database.minConns | int | `2` | Minimum idle connections kept warm per pool (POSTGRESQL_MIN_CONNS). pgxpool floor; replaces the removed maxIdleConns. |
...
All chart tasks live in the repo-root Taskfile under the `helm:`
namespace — one source of truth for humans and CI alike (tool versions
pinned via `versions.env`). Run from anywhere in the repo; Task resolves
the root Taskfile:

```bash
task helm:unittest            # helm unittest suites
task helm:unittest:update     # refresh test snapshots
task helm:lint                # helm + ct + kube-linter + ah + schema lint
task helm:docs                # regenerate this README via helm-docs
task helm:examples            # render every example values file
task helm:gitops-determinism  # byte-identical double-render check
task helm:migrate:test        # migration translator golden tests
task helm:ci                  # full quality gate (what Chart CI runs)
```
