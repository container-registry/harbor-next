...
|---|---|---|
| core, exporter | env vars | nested map flattened by the `harbor.toEnvVars` helper into `UPPER_SNAKE_CASE` env vars, delivered via `envFrom`; `<component>.secret` is the same idea for sensitive keys |
