  ...
  try {
    ...
    if (isCritical && isGsdActive && !warnData.criticalRecorded) {
      try {
        ...
        // Only record (and persist the once-per-session sentinel) when the
        // tool actually exists — otherwise the child dies with
        // MODULE_NOT_FOUND while the sentinel blocks any later retry.
    ...
    const output = {
      hookSpecificOutput: {
        // Echo the event name the runtime actually invoked us with (Claude
        // Code sends PostToolUse, Gemini sends AfterTool). An env-var
        // heuristic like GEMINI_API_KEY misfires when a Claude session
        // happens to export a Gemini key.
