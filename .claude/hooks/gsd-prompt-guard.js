...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Sanitize the basename before echoing it into agent context: the filename
    // is attacker-influenced, and this guard must not itself become an
    // instruction-injection channel.
    const safeName = path.basename(filePath).replace(/[^\w.-]/g, '_').slice(0, 80);
    const output = {
      hookSpecificOutput: {
        hookEventName: 'PreToolUse',
        additionalContext: `\u26a0\ufe0f PROMPT INJECTION WARNING: Content being written to ${safeName} ` +
