...
node -e '
  ...
  if (statePresent === "true") {
    ...
    // This hook only runs when .planning/config.json exists, so an absent
    // STATE.md means an initialized project with missing state — not an
    // uninitialized one.
    headerLines.push("No .planning/STATE.md found - project is initialized but has no recorded state. Suggest /gsd-new-project if starting new work, or restore STATE.md.");
  }
  headerLines.push("");
