...
" "$CMD" 2>/dev/null; then
  # Extract message from -m/--message flag (quoted, --message=, or bare word)
  MSG=""
  if [[ "$CMD" =~ (-m|--message)[[:space:]=]+\"([^\"]+)\" ]]; then
    MSG="${BASH_REMATCH[2]}"
  elif [[ "$CMD" =~ (-m|--message)[[:space:]=]+\'([^\']+)\' ]]; then
    MSG="${BASH_REMATCH[2]}"
  elif [[ "$CMD" =~ (-m|--message)[[:space:]=]+([^[:space:]\"\']+) ]]; then
    MSG="${BASH_REMATCH[2]}"
  fi
  ...
  if [ -n "$MSG" ]; then
    ...
    if ! [[ "$SUBJECT" =~ ^(feat|fix|docs|style|refactor|perf|test|build|ci|chore)(\(.+\))?:[[:space:]].+ ]]; then
      ...
      echo '{"decision": "block", "code": "CONVENTIONAL_COMMITS_VIOLATION", "reason": "Commit message must follow Conventional Commits: <type>(<scope>): <subject>. Valid types: feat, fix, docs, style, refactor, perf, test, build, ci, chore. Subject must be <=72 chars."}'
      exit 2
    fi
