...
The preview version is `2.16.0-beta`: the `VERSION` on `main`, which is the minor release-please is heading for, plus `-beta`. There is one tag, `v2.16.0-beta`, it is both the git tag and the image tag, and it moves every night. No build gets a name of its own.
...
5. `publish-images.yml` builds from the merge commit and pushes `vX.Y.Z-beta`; `release-notes-engine.yml` rewrites that pre-release's body. Both are the reusable workflows the real release calls.
6. After the suite runs, a last job prepends a header to the body: the commit it was built from, the e2e result, and the image digest to pin.
...
There is no date anywhere in the scheme, which is what makes a run repeatable: cutting the same day twice, or skipping a day, just moves the pointer again.

One thing follows from the channel being rebuilt every night: an empty night, where nothing has landed since the last real release, produces no release PR, and the run ends green having published nothing. A failed night is re-run by dispatching the workflow again — there is no per-day state to collide with.

Each nightly publishes one tag, `vX.Y.Z-beta`, moved to whatever was built last. Someone who wants the current preview of the release under development pulls it without having to know a date.

It is mutable, and there is no immutable twin. That is the trade: one name to document, one to point people at, and nothing to prune. What replaces the dated tag is the digest — `harbor-core@sha256:…` is what cosign signs, what the pre-release body prints, and what to put in anything that must not move under you. `git fetch --force --tags` is needed once anyone has fetched the tag before.
...
| Date | `.release-please-manifest.json` | Pointer | Notes cover |
|------|---------------------------------|---------|-------------|
| Sep 18 | `2.15.0` | `v2.16.0-beta` | everything since `v2.15.0` |
| Sep 19 | `2.15.0` | `v2.16.0-beta` moves | everything since `v2.15.0`, plus one day |
| Sep 20 | `2.15.0` | `v2.16.0-beta` moves | everything since `v2.15.0`, plus two days |
...
| Sep 22 | `2.16.0` | `v2.17.0-beta`, a new pointer | everything since `v2.16.0` |

Each release line has one pre-release, on its pointer tag, and that body is rewritten every night. That is the whole published surface: one tag, one Release, per line.
...
Because there is one pre-release rather than one per night, that body is the whole record: it is rewritten from scratch every night, and there is nothing dated to compare it against. The header block at the top is what ties it to a specific build — the commit, the e2e result, and the digest for that run.
