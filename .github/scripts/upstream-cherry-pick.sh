...
BOOTSTRAP_SHA="${BOOTSTRAP_SHA:-c500bbb758ee41a7c8a6ae382583016804a4129f}"
DRY_RUN="${DRY_RUN:-false}"
TARGET_REMOTE="${TARGET_REMOTE:-origin}"
UPSTREAM_BRANCH="${UPSTREAM_BRANCH:-main}"
UPSTREAM_REMOTE="${UPSTREAM_REMOTE:-upstream}"
UPSTREAM_REPO="${UPSTREAM_REPO:-goharbor/harbor}"
...
move_github_changes_to_patch() {
  ...
  git diff --cached --binary -- .github >"${patch_file}"
  git restore --staged --worktree -- .github >/dev/null 2>&1 || true
  git clean -fd -- .github >/dev/null 2>&1 || true
  git add "${patch_file}"
}
...
build_commit_message() {
  ...
  local pr_number=$6
  local author=$7
  local github_patch_file=$8

  {
    if [ -n "${pr_number}" ]; then
      printf 'upstream: %s (%s#%s)\n\n' "${subject}" "${UPSTREAM_REPO}" "${pr_number}"
    else
      printf 'upstream: %s\n\n' "${subject}"
    fi
    ...
    if [ -n "${github_patch_file}" ]; then
      printf 'GitHub-Patch-File: %s\n' "${github_patch_file}"
    fi
  } >"${file}"
}
...
build_pr_body() {
...

  {
    ...
    if [ -n "${pr_number}" ]; then
      ...
      printf 'Cherry-picks upstream Harbor commit `%s`.\n\n' "${upstream_short}"
    fi
    ...
    printf -- '- One upstream commit maps to one Harbor Next PR.\n'
    if [ -n "${github_patch_file}" ]; then
      printf -- '- Changes under `.github/` were written to `%s` because `GITHUB_TOKEN` cannot push workflow file updates.\n' "${github_patch_file}"
      printf -- '- Apply the patch with `git apply %s` if those `.github/` changes should be carried forward.\n' "${github_patch_file}"
    fi
...
cherry_pick_commit() {
  ...
  local github_patch_file=""
  local github_patch_path
  ...
  build_commit_message "${commit_message}" "${sha}" "${upstream_subject}" "$(git show -s --format=%b "${sha}")" "${status}" "${pr_number}" "${upstream_author}" "${github_patch_file}"
  build_pr_body "${pr_body_file}" "${sha}" "${upstream_short}" "${pr_number}" "${upstream_subject}" "${upstream_author}" "${upstream_merged_by}" "${upstream_approvals}" "${upstream_url}" "${status}" "${pr_body}" "${github_patch_file}"
